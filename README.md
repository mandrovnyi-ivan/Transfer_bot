...
Асинхронный Telegram-бот для одного владельца. Бот опрашивает X через self-hosted RSSHub с фолбэком на Nitter, читает RSS СМИ, дедуплицирует новости, извлекает сущности через Anthropic и присылает владельцу уведомления с одной кнопкой: сделать пост.
